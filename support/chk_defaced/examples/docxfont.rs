//! DOCX embedded-font semantic-replacement scan (deterministic, no OCR, no rendering, font-agnostic).
//!
//! Run: cargo run --example docxfont -- <docx>...

use std::path::PathBuf;

use chk_defaced::docx_glyph;
...
fn main() -> anyhow::Result<()> {
    for arg in std::env::args().skip(1) {
        ...
        println!("\n=== {name} ===");
        match docx_glyph::docx_outline_scan(&p) {
