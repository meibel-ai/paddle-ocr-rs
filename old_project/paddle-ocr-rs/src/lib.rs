...
//!
//! Catena di derivazione del **motore di riconoscimento** (det/rec/cls):
//! `RapidOcrOnnx` (C++/OpenCV, riferimento algoritmico) → [`mg-chao/paddle-ocr-rs`] →
//! [`meibel-ai/paddle-ocr-rs`] → questo repo. Tutta la catena e' **Apache-2.0**; i moduli
//! `db_net`, `crnn_net`, `cls_net`/`angle_net`, `ocr_utils`, `scale_param` ne portano avanti
//! la licenza e l'attribuzione.
//!
//! Sono invece **originali** (Copyright (c) 2026 Dario Finardi / Jugaad s.r.l., sviluppati nel
//! workspace `ocr-wasm` e portati qui): `layout`, `model_hub`, `table_classifier`,
//! `table_structure`, `cell_detection`, `doc_unwarp`, `formula_rec`, `compat` e la logica pura
//! in [`pipeline`] (preprocess, spec dei modelli di layout, XY-Cut, associazione linee↔box).
//!
//! **Target: nativo.** La variante wasm della stessa logica pura vive in `ocr-wasm`, dove
//! `pipeline` compila su `wasm32` per la pipeline browser; qui l'inferenza passa da `ort`.
//!
//! [`mg-chao/paddle-ocr-rs`]: https://github.com/mg-chao/paddle-ocr-rs
//! [`meibel-ai/paddle-ocr-rs`]: https://github.com/meibel-ai/paddle-ocr-rs
