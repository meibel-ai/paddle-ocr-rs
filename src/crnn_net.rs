...
///
/// Convenzione (stile Tesseract):
/// - Char alphanumeric (Unicode-aware via [`char::is_alphanumeric`]) →
///   appartengono al word corrente. Lettere accentate EU incluse.
/// - Punteggiatura (non-whitespace, non-alphanumeric) → viene **attaccata
///   alla parola precedente** se esiste, estendendo il bbox. Esempio:
///   `"Hello, world!"` → `["Hello,", "world!"]`.
/// - Whitespace → delimitatore, chiude il word corrente.
///
...
pub(crate) fn selection_to_word_ranges(text: &str, selection: &[CtcSelection]) -> Vec<WordRange> {
    ...
    for (i, ch) in chars.iter().enumerate() {
        if ch.is_alphanumeric() {
            ...
            // Whitespace: chiude il word corrente
            flush(&mut words, &mut buf_text, &mut buf_first, buf_last, buf_score_sum, buf_score_n);
            buf_score_sum = 0.0;
            buf_score_n   = 0;
        } else {
            // Punteggiatura: attacca alla parola adiacente.
            // Se word aperto → si appende (trailing: "Hello,")
            // Se nessun word → apre un nuovo buffer (leading: "(42027")
            buf_text.push(*ch);
            if buf_first.is_none() { buf_first = Some(i); }
            buf_last = i;
            buf_score_sum += selection[i].score;
...
mod tests {
    ...
    fn word_grouping_punctuation() {
        // "Hello, world!" — la "," si attacca a "Hello", il "!" a "world".
        let text = "Hello, world!";
        let selection = sel(&[
    ...
    #[test]
    fn word_grouping_leading_parenthesis() {
        // "(42027 Montecchio)" — la "(" si attacca a "42027", ")" a "Montecchio".
        let text = "(42027 Montecchio)";
        let n = text.chars().count();
        let selection: Vec<CtcSelection> = (0..n)
            .map(|i| CtcSelection { timestep: i, score: 0.9 })
            .collect();
        let words = selection_to_word_ranges(text, &selection);
        assert_eq!(words.len(), 2);
        assert_eq!(words[0].text, "(42027");
        assert_eq!(words[1].text, "Montecchio)");
    }
}
