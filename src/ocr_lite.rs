...
impl OcrLite {
    ...
    pub fn detect_with_layout(
        ...
        // ── Step 3+4: associazione + ordering GENERICI ──────────────────
        // Delegati a `crate::pipeline::layout::associate_lines`: la stessa
        // implementazione che il wasm usa con le linee di tesseract.js.
        // Qui le "linee" sono i TextBlock PP-OCR; serve solo il loro bbox
        // (centroide = centro del bbox axis-aligned del polygon — per i
        // quad quasi-orizzontali coincide col vecchio polygon_centroid).
        let line_bboxes: Vec<crate::pipeline::BoundingBox> = ocr.text_blocks.iter()
            .map(|tb| bbox_of_points(&tb.box_points))
            .collect();
        let shared_boxes = crate::layout::to_shared_boxes(&layout_boxes);
        let assoc = crate::pipeline::layout::associate_lines(&line_bboxes, &shared_boxes);

        let mut slots: Vec<Option<TextBlock>> =
            ocr.text_blocks.into_iter().map(Some).collect();
        let blocks: Vec<TextBlockWithLayout> = assoc.order.iter().map(|&i| {
            let bb = line_bboxes[i];
            TextBlockWithLayout {
                block: slots[i].take().expect("text-block già consumato"),
                layout_index: assoc.line_to_box[i],
                distance: assoc.distance[i],
                centroid_x: ((bb.left + bb.right).max(0) / 2) as u32,
                centroid_y: ((bb.top + bb.bottom).max(0) / 2) as u32,
            }
        }).collect();
...
/// Bbox axis-aligned (min/max) di un polygon a N punti, nello spazio
/// `ocr_pipeline` (i32). Usato per l'associazione generica condivisa.
fn bbox_of_points(points: &[Point]) -> crate::pipeline::BoundingBox {
    if points.is_empty() {
        return crate::pipeline::BoundingBox::default();
    }
    let xs = || points.iter().map(|p| p.x as i32);
    let ys = || points.iter().map(|p| p.y as i32);
    crate::pipeline::BoundingBox::from_lrtb(
        xs().min().unwrap_or(0),
        ys().min().unwrap_or(0),
        xs().max().unwrap_or(0),
        ys().max().unwrap_or(0),
